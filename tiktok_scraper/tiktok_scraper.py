...
view_counts = [view_count_element.text for view_count_element in video_counts]
...
for i in range(len(links)):
    ...
    try:
        ...
        if temp_text[-1] == 'K':
            temp_text = float(temp_text[:-1]) * 1000
        elif temp_text[-1] == 'M':
            temp_text = float(temp_text[:-1]) * 1000000
        temp_list.append(int(temp_text))
